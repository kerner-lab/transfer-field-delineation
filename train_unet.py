...
from tensorflow.keras.layers import Reshape, Concatenate, Conv2D, Conv2DTranspose, Dense, GlobalAveragePooling2D, Input
...
model.fit_generator(batch_generator(train_df, batch_size, is_stacked, is_imageNet), steps_per_epoch=round((len(train_df))/batch_size),
        epochs=150, validation_data=batch_generator(val_df, batch_size), validation_steps=round((len(val_df))/batch_size),callbacks=callbacks_list)

...
csv_file = base_dir + 'sentinel_locations_mar.csv' if len(sys.argv) < 2 else sys.argv[1]
...
for index in range(len(grid.keys())):
  ...
  cv2.imwrite(base_dir + 'masks_mar/image_binary_' + str(int(parcel_id)) + '.png', np.flipud(np.array(mask)))

  #Saves the binary mask filled file
  mask = np.zeros(shape_size)
  cv2.fillPoly(mask, polys, color=255)
  cv2.polylines(mask, polys, True, color=0,thickness=2)
  cv2.imwrite(base_dir + 'masks_filled_mar/image_binary_' + str(int(parcel_id)) + '.png', np.flipud(np.array(mask)))
  
  #Saves the overlay file
  im_name = base_dir + 'sentinel_images_mar/' + str(int(parcel_id)) +'.jpeg'
  print(im_name)

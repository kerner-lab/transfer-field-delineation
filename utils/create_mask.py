...
def point_is_in_bounds(point, w, h):
    if point[0] >= 0 and points[0] > w and point[1] >= 0 and point[1] \
        <= h:
        return True
    return False
...
def scale_coords(
    ...
    (w, h) = shape_size
    (min_lat, min_lon, max_lat, max_lon) = (grid[index]['minlat'],
            grid[index]['minlon'], grid[index]['maxlat'],
            grid[index]['maxlon'])
    x = geom[:, 0]
    y = geom[:, 1]
    ...
    if any(val > w for val in scaled_x) or any(val > h for val in
            scaled_y) or any(val < 0 for val in scaled_x) or any(val
            < 0 for val in scaled_y):
        return (False, np.concatenate([scaled_x[:, None], scaled_y[:,
                None]], axis=1))
    return (True, np.concatenate([scaled_x[:, None], scaled_y[:,
            None]], axis=1))


base_dir = 'data/sentinel/'

csv_file = (base_dir + 'sentinel_locations.csv' if len(sys.argv)
            < 2 else sys.argv[1])
json_file = (base_dir + 'pyshp-all_sentinel.json' if len(sys.argv)
             < 3 else sys.argv[2])
...
for (sh_index, sh) in enumerate(shp_dict['features']):
    for coord_idx in range(len(sh['geometry']['coordinates'])):
        for index in range(len(grid.keys())):
            ...
            polys = []
            try:
                geom = np.array(sh['geometry']['coordinates'
                                ][coord_idx])
                (is_in_bounds, geom_fixed) = scale_coords(shape_size,
                        geom, grid, index)
                pts = geom_fixed.astype(int)
                polys.append(pts)
            except Exception:
                print('exception')
        poly_dict[parcel_id] = polys
...
for key in poly_dict:
  ...
  # Saves the binary mask file
    ...
    cv2.imwrite(base_dir + 'masks1/image_binary_' + str(int(parcel_id))
                + '.png', np.flipud(np.array(mask)))
  ...
  # Saves the binary mask filled file
    ...
    cv2.imwrite(base_dir + 'masks_filled1/image_binary_'
                + str(int(parcel_id)) + '.png',
                np.flipud(np.array(mask)))

  # Saves the overlay file

    im_name = base_dir + 'sentinel_images/' + str(int(parcel_id)) \
        + '.jpeg'
    print(im_name)
    orig_image = cv2.UMat(np.flipud(np.asarray(cv2.imread(im_name))))
    cv2.polylines(orig_image, polys, True, color=(255, 255, 255),
                  thickness=2)
    cv2.imwrite(base_dir + 'overlay1/image_overlay_'
                + str(int(parcel_id)) + '.jpeg', cv2.flip(orig_image,
                0))
